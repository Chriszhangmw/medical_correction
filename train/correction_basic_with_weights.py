...
'''
这个版本得代码是，是完全基于Bert得预训练模型做预测，将特定位置坐mask，然后用Bert直接坐预测，这里坐预测得时候结合了一些现有得
语料集，比如发音和字形比较相似得，这一部分占得权重占0.4，其中发音又是占0.4中得0.8，字形占0.4中得0.2，再结合Bert预测出来得topK
一期计算出最终得topK个候选词，这里相当于应用了beam search得思路
